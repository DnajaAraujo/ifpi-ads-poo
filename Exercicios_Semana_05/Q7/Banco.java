...
public class Banco {
	...
	double contarDinheiroTotal() {
		...
		for (int i = 0; i < contas.length; i++) {
			if (contas[i] != null) {
				qtdDinheiro += contas[i].getSaldo();    // Sofreu alteração!
			}	
		}
		return qtdDinheiro;
	}
	...
	Conta consultar(String numero) {
		...
		for (int i = 0; i < indice; i++) {
			if (contas[i].getNumero().equals(numero)) {   // Sofreu alteração!
				c = contas[i];
				break;
			}
		}
	...
	void alterar(Conta c) {
		int indice = consultarIndice(c.getNumero());   // Sofreu alteração!
		if (indice != -1) {
			contas[indice] = c;
		}
